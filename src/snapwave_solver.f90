...
contains
   ...
   subroutine compute_wave_field(time)
      ...
      call solve_energy_balance2Dstat(x, y, dhdx, dhdy, no_nodes, msk, &
                                      ...
                                      ig, kwav_ig, cg_ig, H_ig, ctheta_ig, Hmx_ig, ee_ig, fw_ig, &
   ...
   subroutine solve_energy_balance2Dstat(x, y, dhdx, dhdy, no_nodes, msk, &
      ...
      integer*1, dimension(no_nodes), intent(inout) :: msk ! mask of grid points (0 is dry, 1 is inner, 2 boundary,3 neumann)
      ...
      integer, intent(in) :: upwindref  ! upwind refraction (1) or central (0)
      ...
      real*4, parameter :: hmin = 0.1 ! minimum water depth
      real*4, parameter :: fac = 1.0 ! underrelaxation factor for DoverA
      ...
      allocate (srcsh(ntheta, no_nodes)); srcsh = 0.0
      !
      do itheta = 1, ntheta
      ...
      do sweep = 1, 4
         ...
         !$omp parallel do private(k)
         do k = 1, no_nodes
            ra(k) = x(k) * cos(thetamean + 0.5 * pi * shift(sweep)) + y(k) * sin(thetamean + 0.5 * pi * shift(sweep))
         end do
         !$omp end parallel do
      ...
      !
      ! Set inner to false for all points at grid edge or adjacent to dry point - SHOUKLD BE DONE BEFORE AND STORED IN MSK
      !omp parallel do private(k, itheta, k1, k2)
      !do k = 1, no_nodes
      !   do itheta = 1, ntheta
      !      !
      !      k1 = prev(1, itheta, k)
      ...
      !
      !$omp parallel do private(k, Ak, itheta, k1,k2,beta, betan,fbr, fsh, Ek,Hk,uorbi,Dfk,Dwt,Dwak)
      do k = 1, no_nodes
         !if (.not. inner(k)) then
         if (msk(k)==2) then
            !
         ...
         if (msk(k)==1) then
            ...
            if (ig == 1) then
               do itheta = 1, ntheta
                  ...
                  if (k1 * k2 > 1) then
                     !
                     beta = max((w(1, itheta, k) * (depth(k1) - depth(k)) + w(2, itheta, k) * (depth(k2) - depth(k))) / max(ds(itheta, k),epsdist), 0.0)
                     betan = (beta / sigm_ig(k)) * sqrt(9.81 / max(depth(k), 0.1))
                     fbr = 1.0
                     fsh = fbr * exp(-4.0 * sqrt(betan))
                     !
                     cgprev(itheta) = w(1, itheta, k) * cg_ig(k1) + w(2, itheta, k) * cg_ig(k2)
                     !
                     srcsh(itheta, k) = -shinc2ig * fsh * ((cg(k) - cgprev(itheta)) / max(ds(itheta, k),epsdist))
               ...
               if (depth(k) > 1.1 * hmin) then
                  ...
                  if (.not. ok(k) == 1) then
                     ...
                     do itheta = 1, ntheta
                        ...
                        R(itheta) = oneoverdt * ee(itheta, k) + cgprev(itheta) * eeprev(itheta) / max(ds(itheta, k),epsdist)
                     end do
                     !
                     if (upwindref == 0) then  
                        ! central scheme
                        ...
                        do itheta = 2, ntheta - 1
                           ...
                           B(itheta) = oneoverdt + cg(k) / max(ds(itheta, k),epsdist) + DoverE(k) + srcsh(itheta, k)
                           C(itheta) = ctheta(itheta + 1, k) * oneover2dtheta
                           !
                        end do
                        !
                        A(1) = -ctheta(ntheta, k) * oneover2dtheta
                        B(1) = oneoverdt + cg(k) / max(ds(1, k),epsdist) + DoverE(k) + srcsh(1, k)
                        C(1) = ctheta(2, k) * oneover2dtheta
                        !
                        A(ntheta) = -ctheta(ntheta - 1, k) * oneover2dtheta
                        B(ntheta) = oneoverdt + cg(k) / max(ds(ntheta, k),epsdist) + DoverE(k) + srcsh(ntheta, k)
                        ...
                        do itheta = 2, ntheta - 1
                           if (ctheta(itheta,k)<0) then
                              A(itheta)=0.d0
                              B(itheta)=oneoverdt-ctheta(itheta,k) * oneoverdtheta+cg(k)/max(ds(itheta, k),epsdist)+DoverE(k) + srcsh(itheta, k)
                        ...
                        if (ctheta(1,k)<0) then
                           A(1)=0.d0
                           B(1)=oneoverdt-ctheta(1,k) * oneoverdtheta+cg(k)/max(ds(1, k),epsdist)+DoverE(k) + srcsh(1, k)
                        ...
                        if (ctheta(ntheta,k)>0) then
                           ...
                           B(ntheta)=oneoverdt+ctheta(ntheta,k) * oneoverdtheta+cg(k)/max(ds(ntheta, k),epsdist)+DoverE(k) + srcsh(ntheta, k)
                           C(ntheta)=0
                        else
                           A(ntheta)=0.d0
                           B(ntheta)=oneoverdt
                     ...
                     if (wind == 1) then
                        do itheta = 1, ntheta
                           R_aa(itheta) = (oneoverdt) * aa(itheta, k) + cgprev(itheta) * aaprev(itheta) / max(ds(itheta, k),epsdist)
                        end do
                        
                        if (upwindref ==0) then
                           do itheta = 1, ntheta
                              B_aa(itheta) = oneoverdt + cg(k) / max(ds(itheta, k),epsdist) + DoverA(k)
                           end do
                        else
                           do itheta = 2, ntheta - 1
                              if (ctheta(itheta,k)<0) then
                                 B_aa(itheta) = oneoverdt -ctheta(itheta,k) * oneoverdtheta + cg(k) / max(ds(itheta, k),epsdist) + DoverA(k)
                              else
                                 B_aa(itheta) = oneoverdt +ctheta(itheta,k) * oneoverdtheta + cg(k) / max(ds(itheta, k),epsdist) + DoverA(k)
                              endif
                           end do
                           !
                           if (ctheta(1, k) < 0) then
                              B_aa(1) = oneoverdt - ctheta(1, k) / dtheta + cg(k) / max(ds(1, k),epsdist) + DoverA(k)
                           else
                              B_aa(1) = oneoverdt 
                              R_aa(1) = 0.d0
                           end if
                           !
                           if (ctheta(ntheta, k) > 0) then
                              B_aa(ntheta) = oneoverdt + ctheta(ntheta, k) / dtheta + cg(k) / max(ds(ntheta, k),epsdist) + DoverA(k)
                           else
                     ...
                     if (ig == 1) then
                        ...
                        if (Hk_ig > 0.2 * Hmx_ig(k)) then
                           call baldock(rho, g, alfa, gamma, depth(k), Hk_ig, T_ig(k), 1, Dwk_ig, Hmx_ig(k))
                        else
                           Dwk_ig = 0.
                        end if
                        !
                        ...
                        do itheta = 1, ntheta
                           !
                           R_ig(itheta) = oneoverdt * ee_ig(itheta, k) + cgprev_ig(itheta) * eeprev_ig(itheta) / max(ds(itheta, k),epsdist) + srcsh(itheta, k) * ee(itheta, k)
                           !
                        end do
                        !
                        do itheta = 2, ntheta - 1
                           ...
                           B_ig(itheta) = oneoverdt + cg_ig(k) / max(ds(itheta, k),epsdist) + DoverE_ig(k)
                        ...
                        if (ctheta_ig(1, k) < 0) then
                           A_ig(1) = 0.0
                           B_ig(1) = oneoverdt - ctheta_ig(1, k) / dtheta + cg_ig(k) / max(ds(1, k),epsdist) + DoverE_ig(k)
                           C_ig(1) = ctheta_ig(2, k) / dtheta
                        else
                           A_ig(1) = 0.0
                           B_ig(1) = 1.0 / dt + cg_ig(k) / max(ds(1, k),epsdist) + DoverE_ig(k)
                           C_ig(1) = 0.0
                        end if
                        !
                        if (ctheta_ig(ntheta, k) > 0) then
                           A_ig(ntheta) = -ctheta_ig(ntheta - 1, k) / dtheta
                           B_ig(ntheta) = oneoverdt + ctheta_ig(ntheta, k) / dtheta + cg_ig(k) / max(ds(ntheta, k),epsdist) + DoverE_ig(k)
                           C_ig(ntheta) = 0.0
                        else
                           A_ig(ntheta) = 0.0
                           B_ig(ntheta) = oneoverdt + cg_ig(k) / max(ds(ntheta, k),epsdist) + DoverE_ig(k)
                           C_ig(ntheta) = 0.0
                        end if
                        !
                        ! Solve tridiagonal system per point
      ...
      do k = 1, no_nodes
         ...
         if (depth(k) > 1.1 * hmin) then
            ...
            !n = cg(k)/sig(k)*kwav(k)
            !Sxx(k) = ( n*sum((1.+ costh**2)* ee(:,k))-.5*sum( ee(:,k)) ) * dtheta
            !Syy(k) = ( n*sum((1.+ sinth**2)* ee(:,k))-.5*sum( ee(:,k)) ) * dtheta
            !Sxy(k) =   n*sum( sinth* costh* ee(:,k) ) * dtheta
            !
            !
            if (ja_vegetation == 1) then
